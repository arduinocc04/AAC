...
\egTrace는 distSegDot() 등 곡선을 선분으로 분할하는 함수를 사용하여 이미지의 외곽선을 여러 선분으로 잘게 잘라 벡터화한다.
...
우선 \libpng가 제공하는 일련의 이미지 입출력 과정이 번거로워 이미지를 쉽고 빠르게 입출력할 수 있도록 \libpng의 메서드를 한데 모아 사용자 라이브러리를 만들었다.
처음에는 \libpng를 제외하고 아무런 오픈 소스 라이브러리를 사용하지 않으려고 하였으나, \libpng만으로는 이미지 변환 및 연산에 한계를 느껴 \Eigen을 도입하였다.
